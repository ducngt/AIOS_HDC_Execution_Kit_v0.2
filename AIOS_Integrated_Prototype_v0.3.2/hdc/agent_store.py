...
from hdc.secret_store import get_secret, has_secret, set_secret
...
def _provider_dict(row: sqlite3.Row) -> dict[str, Any]:
    ...
    d["configured"] = has_secret(env_name)
    d["secret_value"] = None
    return d
...
def save_provider_secret(provider_id: str, api_key: str) -> dict[str, Any]:
    """Persist/replace a provider API key server-side without exposing it."""
    ensure_seeded()
    ...
    if not env_name:
        raise ValueError("provider_secret_name_missing")
    set_secret(env_name, api_key)
    return {"provider_id": provider["provider_id"], "configured": has_secret(env_name), "saved": True}
...
def _call_provider(provider: dict[str, Any], model: str, system: str, prompt: str) -> str:
    ...
    key = get_secret(env_name) if env_name else None
    if not key:
